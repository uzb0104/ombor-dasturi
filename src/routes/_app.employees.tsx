...
function EmployeesPage() {
  ...
  const totalSalary = employees.filter(e => e.status === "Faol").reduce((a,e) => a + e.salary, 0);
  const totalAdvance = employees.reduce((a,e) => a + e.advance, 0);

  const submit = () => {
    if (!form.name) { toast.error("Ism majburiy"); return; }
    if (editing) { updateEmployee(editing, { ...form, role: form.role as any }); toast.success("Yangilandi"); }
    else {
      addEmployee({ id: `emp_${Math.random().toString(36).slice(2,9)}`, ...form, role: form.role as any, advance: 0, status: "Faol" });
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <StatCard label="Oylik jami (faol)" value={formatSom(totalSalary)} icon={Wallet} accent="info" />
