...
function BarcodePage() {
  ...
  const found = useMemo(
    () => products.find(p => (p.barcode && p.barcode === code) || p.name.toLowerCase() === code.toLowerCase()),
    [products, code],
  );
  const withoutBarcode = useMemo(() => products.filter(p => !p.barcode), [products]);

  // Auto-focus the scan input so a USB scanner's "keyboard input" lands here
  ...
  const handleScanKey = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      ...
      else toast.error("Bu barkod bilan tovar topilmadi");
      setScanBuffer("");
    }
  };

  const generateBarcode = () => setNewBarcode(`486${Math.floor(100000000 + Math.random() * 899999999)}`);

  const assignBarcode = () => {
    if (!assignFor || !newBarcode.trim()) { toast.error("Barkod kiriting"); return; }
    if (products.some(p => p.barcode === newBarcode.trim() && p.id !== assignFor)) {
      toast.error("Bu barkod boshqa tovarga tegishli"); return;
    }
    updateProduct(assignFor, { barcode: newBarcode.trim() });
    toast.success("Barkod biriktirildi");
    setAssignFor(null); setNewBarcode("");
  };

  return (
    <div className="space-y-5">
      <PageHeader title="Barkod" subtitle="Skaner, qo'lda kiritish va barkodsiz tovarlarni boshqarish" />

      <Tabs defaultValue="scan">
        <TabsList>
          <TabsTrigger value="scan">Skanerlash</TabsTrigger>
          <TabsTrigger value="search">Qidirish</TabsTrigger>
          <TabsTrigger value="without">Barkodsiz tovarlar ({withoutBarcode.length})</TabsTrigger>
        </TabsList>

        <TabsContent value="scan" className="mt-4">
          <Card className="p-6 rounded-2xl">
            <Label>Skaner kiritish maydoni</Label>
            <p className="text-xs text-muted-foreground mb-2">USB skaner avtomatik kiritadi. Yoki qo'lda yozib Enter bosing.</p>
            <div className="flex gap-2 max-w-xl">
              <Input
                ref={scanRef}
                autoFocus
                className="font-mono"
                placeholder="Barkodni skanerlang yoki yozing..."
                value={scanBuffer}
                onChange={(e) => setScanBuffer(e.target.value)}
                onKeyDown={handleScanKey}
              />
              <Button onClick={() => { setScanBuffer(s => s); handleScanKey({ key: "Enter" } as any); }}>
          ...
          <Card className="p-6 rounded-2xl">
            ...
            <div className="flex gap-2 max-w-xl mt-2">
              <div className="relative flex-1">
                ...
                <Input className="pl-9 font-mono" placeholder="Barkod yoki tovar nomi..." value={code} onChange={(e) => setCode(e.target.value)} />
              </div>
            </div>
            {code && found && (
              <div className="mt-5 p-5 rounded-xl border bg-muted/40 animate-fade-in">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
                  <div><div className="text-xs text-muted-foreground">Barkod</div><div className="font-mono text-sm">{found.barcode || "—"}</div></div>
        ...
        <TabsContent value="without" className="mt-4">
          <Card className="p-4 rounded-2xl">
            {withoutBarcode.length === 0 ? (
              <div className="text-center py-10 text-muted-foreground">Barcha tovarlarda barkod mavjud</div>
            ) : (
                ...
                {withoutBarcode.map(p => (
                  <div key={p.id} className="flex items-center gap-3 p-3 rounded-xl border bg-card">
                    ...
                    <Button size="sm" variant="outline" onClick={() => { setAssignFor(p.id); setNewBarcode(""); }}>
                      <Plus className="h-3 w-3 mr-1" />Barkod
                    </Button>
                  </div>
                ))}
              </div>
      ...
      <Dialog open={!!assignFor} onOpenChange={(v) => !v && setAssignFor(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>Barkod biriktirish</DialogTitle></DialogHeader>
          <Label>Barkod</Label>
          <div className="flex gap-2">
            <Input className="font-mono" value={newBarcode} onChange={(e) => setNewBarcode(e.target.value)} placeholder="Skanerlash yoki yozish" autoFocus />
