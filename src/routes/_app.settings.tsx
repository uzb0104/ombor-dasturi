          ...
          <Card className="p-6 rounded-2xl space-y-4 max-w-2xl">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Input value={user?.role ? roleLabel(t, user.role) : ""} disabled />
              </div>
              <div>
...
function UsersManagement({
  ...
  const removeUser = async (u: AppUser) => {
    const ok = await confirm({
      ...
      description: t("common.deleteQuestion", { name: u.name }),
      destructive: true,
      ...
      <div className="flex items-center justify-between p-4 border-b">
        ...
        >
          ...
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>{editing ? t("settings.editUser") : t("settings.newUser")}</DialogTitle>
            </DialogHeader>
                    ...
                    <button
                      ...
                      onClick={() =>
                        setForm((f) => ({
                          ...f,
                          permissions:
                            f.permissions.length === ALL_PERMISSIONS.length ? [] : ALL_PERMISSIONS,
                        }))
                      }
                    >
                      {form.permissions.length === ALL_PERMISSIONS.length
                        ? t("settings.clearAllPerms")
                        : t("settings.selectAllPerms")}
                    </button>
                  )}
                </div>
                <div
                  className={`grid grid-cols-2 md:grid-cols-3 gap-2 ${form.role === "Admin" ? "opacity-50 pointer-events-none" : ""}`}
                >
                  {PERMISSION_MODULES.map((m) => (
                    <label
                      key={m.path}
                      className="flex items-center gap-2 rounded-lg border p-2 text-sm cursor-pointer hover:bg-muted/40"
                    >
                      <Checkbox
                        checked={form.permissions.includes(m.path)}
                        onCheckedChange={() => togglePerm(m.path)}
                      />
            ...
            {appUsers.map((u) => (
              <TableRow key={u.id}>
                ...
                <TableCell className="text-xs text-muted-foreground">
                  {u.role === "Admin"
                    ? t("settings.permsAll")
                    : t("settings.permsCount", {
                        n: u.permissions.length,
...
function BrandsManagement({
  ...
  const addBrand = () => {
    ...
    if (!n) {
      toast.error(t("settings.brandNameRequired"));
      return;
    }
    if (brands.includes(n)) {
      toast.error(t("settings.brandExists"));
      return;
    }
    add(n);
    setNewBrand("");
    toast.success(t("settings.brandAdded"));
  };

  const saveEdit = (oldName: string) => {
    const n = editValue.trim();
    if (!n) return;
    if (n !== oldName && brands.includes(n)) {
      toast.error(t("settings.brandExists"));
      return;
    }
    update(oldName, n);
      ...
      <div className="p-4 border-b">
        ...
        <div className="flex flex-col sm:flex-row gap-2 mt-3 max-w-lg">
          <Input
            ...
            placeholder={t("settings.brandPh")}
            ...
            <div key={b} className="flex items-center gap-2 rounded-lg border p-2 bg-card">
              {isEditing ? (
                ...
                <>
                  ...
                  <Button variant="ghost" size="icon" onClick={() => removeBrand(b)}>
                    <Trash2 className="h-4 w-4 text-destructive" />
                  </Button>
                </>
              )}
...
function BranchesManagement({
  ...
  const addBranchFn = () => {
    ...
    if (branches.includes(n)) {
      toast.error(t("settings.branchExists"));
      return;
    }
    add(n);
    setNewBranch("");
  ...
  const saveEdit = (oldName: string) => {
    const n = editValue.trim();
    if (!n) return;
    if (n !== oldName && branches.includes(n)) {
      toast.error(t("settings.branchExists"));
      return;
    }
    update(oldName, n);
  ...
  const removeBranch = async (b: string) => {
    if (branches.length <= 1) {
      toast.error(t("settings.branchMin"));
      return;
    }
    const ok = await confirm({
      title: t("settings.deleteBranch"),
      description: t("settings.branchDelete", { name: b }),
      destructive: true,
      confirmText: t("common.delete"),
    });
    if (!ok) return;
      ...
      <div className="p-4 border-b">
        ...
        <div className="flex flex-col sm:flex-row gap-2 mt-3 max-w-lg">
          <Input
            ...
            placeholder={t("settings.branchPh")}
            onKeyDown={(e) => e.key === "Enter" && addBranchFn()}
          />
          <Button onClick={addBranchFn}>
            <Plus className="h-4 w-4 mr-1" />
            {t("common.add")}
          </Button>
        </div>
            ...
            <div key={b} className="flex items-center gap-2 rounded-lg border p-2 bg-card">
              {isEditing ? (
                ...
                <>
                  ...
                  <Button variant="ghost" size="icon" onClick={() => removeBranch(b)}>
                    <Trash2 className="h-4 w-4 text-destructive" />
                  </Button>
                </>
              )}
