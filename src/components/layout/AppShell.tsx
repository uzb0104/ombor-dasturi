  ...
  const sidebar = (
    <aside className="flex h-full w-64 flex-col bg-sidebar text-sidebar-foreground">
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 scrollbar-thin">
        <div className="px-2 pb-2 text-[10px] font-semibold uppercase tracking-wider opacity-50">
          {t("nav.menu")}
        </div>
        {NAV.map((item) => {
        ...
        <header className="flex items-center gap-3 border-b bg-card/80 backdrop-blur px-4 py-3">
          ...
          <div className="relative hidden md:block w-72" onClick={() => setSearchOpen(true)}>
            ...
            <Input
              ...
              className="pl-9 bg-muted/40 border-transparent cursor-pointer hover:bg-muted/60 focus-visible:bg-background"
            />
          </div>
    ...
    <Select value={current} onValueChange={(v) => setLang(v as Lang)}>
      <SelectTrigger className="h-9 w-auto gap-1 px-2" aria-label="Til">
        ...
        <span className="hidden sm:inline text-xs">
          {LANG_LABELS[current as keyof typeof LANG_LABELS]}
        </span>
      </SelectTrigger>
